...
class FedDynMulti(Server):
    ...
    def train(self):
        ...
        assert torch.cuda.is_available(), "No GPU available for parallel training"

        # Calculate clients per GPU
        num_gpus = torch.cuda.device_count()  # Get total available GPUs
        assert num_gpus >= 2, "The program requires at least 2 GPUs for parallel training"
        
        logger.info(f"Number of GPUs: {num_gpus}")
        
        clients_per_gpu = len(self.selected_clients) // num_gpus
        gpu_clients = [
            self.selected_clients[i * clients_per_gpu: (i + 1) * clients_per_gpu]
            for i in range(num_gpus)
        ]
        
        logger.info(f"Clients per GPU: {clients_per_gpu}")

        # Handle remaining clients
        if len(self.selected_clients) % num_gpus != 0:
            gpu_clients[-1].extend(self.selected_clients[num_gpus * clients_per_gpu:])

        def train_clients_on_gpu(clients, gpu_id):
            """Train a group of clients on a specific GPU."""
        ...
        for i in tqdm(range(self.global_rounds + 1), desc="Global rounds"):
            ...
            threads = []
            for gpu_id, clients in enumerate(gpu_clients):
                thread = Thread(target=train_clients_on_gpu, args=(clients, gpu_id))
